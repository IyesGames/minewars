...
pub struct GameInput<Io: GameIo> {
    pub plid: PlayerId,
    pub subplid: u8,
    pub input: Io::InputAction,
}

pub struct GameOutput<Io: GameIo> {
    pub plids: Plids,
    pub output: Io::OutEvent,
}

impl<Io: GameIo> From<(Plids, Io::OutEvent)> for GameOutput<Io> {
    fn from((plids, output): (Plids, Io::OutEvent)) -> Self {
        Self { plids, output }
    }
}

pub trait GameIo: Sized + Send + Sync + 'static {
...
pub trait Host<Io: GameIo>: Sized + Send + Sync + 'static {
    /// Notify the Host about something that happened in the game world
    fn msg(&mut self, output: GameOutput<Io>);
    /// Request an action to occur at a specific future time
    fn sched(&mut self, time: std::time::Instant, event: Io::SchedEvent);
    /// Cancel scheduled events equal to the value given
    fn desched_all(&mut self, event: Io::SchedEvent);
...
pub trait Game: Sized + Send + Sync + 'static {
    ...
    fn init<H: Host<Self::Io>>(&mut self, host: &mut H, initdata: Box<Self::InitData>);

    /// Trigger a timer-driven event in the game
    fn unsched<H: Host<Self::Io>>(&mut self, host: &mut H, event: <Self::Io as GameIo>::SchedEvent);

    /// Process a player input
    fn input<H: Host<Self::Io>>(&mut self, host: &mut H, input: GameInput<Self::Io>);

    /// Query the game for data / status updates to put in an unreliable datagram
    fn unreliable<H: Host<Self::Io>>(&mut self, _host: &mut H) {}
